...
async function getAllReviews(anime_name) {
	try {
		const response = await fetch(
			`https://irinochc71.execute-api.us-east-1.amazonaws.com/review/${anime_name}`
		);
		const data = await response.json();
		const review = data.Items;
		return review;
	} catch (error) {
...
async function getMyReview(anime_name, username) {
	try {
		const response = await fetch(
			`https://irinochc71.execute-api.us-east-1.amazonaws.com/review/${anime_name}/${username}`
		);
		const data = await response.json();
...
function addReview(event, all) {
	...
	request.open(
		"POST",
		`https://irinochc71.execute-api.us-east-1.amazonaws.com/review/${jsonData.anime_name}/${jsonData.username}`,
		true
	);
...
function editReview(event) {
	...
	request.open(
		"PUT",
		`https://irinochc71.execute-api.us-east-1.amazonaws.com/review/${animeName}/${username}`,
		true
	);
	request.onload = function () {
		response = JSON.parse(request.responseText);
...
function deleteReview() {
	...
	request.open(
		"DELETE",
		"https://irinochc71.execute-api.us-east-1.amazonaws.com/review",
		true
	);
	request.onload = function () {
		response = JSON.parse(request.responseText);
		if (response.message == "review delete") {
...
function deleteAllReview() {
	...
	request.open(
		"DELETE",
		"https://irinochc71.execute-api.us-east-1.amazonaws.com/allReview",
		true
	);
	request.onload = function () {
		response = JSON.parse(request.responseText);
...
function getReviewPfp(username) {
	let request = new XMLHttpRequest();
	request.open(
		"GET",
	...
	request.onload = function () {
		...
		if (response.length != 0) {
			sessionStorage.setItem(
				"reviewPfp",
				JSON.parse(this.response)[0].profilePic
			);
		} else {
...
async function loadReivew(all) {
	...
	try {
		...
		for (let i = 0; i < num; i++) {
			...
			await getReviewPfp(reviews[i].username);
            ...
            <div class="pfp-con">
                <img
						src="${sessionStorage.getItem("reviewPfp")}"
                    alt=""
                />
            </div>
