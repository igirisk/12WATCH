	...
	(w3.displayObject = function (id, data) {
		...
		for (j = l - 1; j >= 0; j -= 1) {
			...
			if (
				...
				for (x in repeatObj) {
					i += 1;
					myRowClone = arr[j];
					myRowClone = w3_replace_curly(
						myRowClone,
						"element",
						repeatX,
						repeatObj[x]
					);
					a = myRowClone.attributes;
					...
					i === repeatObj.length
						? arr[j].parentNode.replaceChild(myRowClone, arr[j])
						: arr[j].parentNode.insertBefore(myRowClone, arr[j]);
				}
			} else {
				console.log(
		...
		function w3_replace_curly(elmnt, typ, repeatX, x) {
			let value,
				...
				cc,
				r;
			myRowClone = elmnt.cloneNode(true);
			pos1 = 0;
			while (pos1 > -1) {
				originalHTML =
					typ == "attribute" ? myRowClone.value : myRowClone.innerHTML;
				...
				if (value != undefined) {
					r = "{{" + lookFor + "}}";
					if (typ == "attribute") {
						myRowClone.value = myRowClone.value.replace(r, value);
					} else {
						w3_replace_html(myRowClone, r, value);
					}
				}
				pos1 = pos1 + 1;
			}
